...
import com.mraleksmay.projects.download_manager.client.gui.platform.model.plugin.model.download.BaseDownloadFormatter;
...
import com.mraleksmay.projects.download_manager.client.gui.platform.view.dialog.download.controller.TableController;
...
public class AddDisplayDownloadsDialog extends ADialog {
    ...
    private List<DownloadEntityInformation> getDownloadsFromUrls(List<String> urlsList) {
        ...
        for (String urlStr : urlsList) {
            ...
            try {
                name = DownloadUtil.suggestNameFrom(urlStr);
                if (name.length() > 100) name = name.substring(0, 100);
                url = new URL(urlStr);
                category = getApplicationManager().getPluginsManager().getCategoryBy("CORE", "UNFINISHED", "OTHER");

                final Download download = new BaseDownload(name, url, category.getOutputDir(), "file", authData, category, 0);
                download.setFormatter(new BaseDownloadFormatter());
